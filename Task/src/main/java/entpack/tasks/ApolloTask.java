...
public class ApolloTask implements StatefulJob {

    private static Logger logger = LoggerFactory.getLogger("apollo_task");

    @Override
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        if (TaskUtil.getStopStatus("ApolloTask")) {
            logger.info("ApolloTask.execute stop! Time:{} ", DateUtil.formatDate(new Date()));
            return;
        }

        String runTimeKey = "api:Task:apollo:runTime";
    ...
    public void doItem() {
        ...
        for (Record record : recordList) {
            ...
            String key = "lock:apollo::push:" + betId;
