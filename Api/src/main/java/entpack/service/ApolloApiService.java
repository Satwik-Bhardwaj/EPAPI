...
public class ApolloApiService extends BaseService {

    private static String tname = "api_apollo";
    ...
    public static void updateTicket(String uuid, int status, String msg) {
        if (msg == null) {
            String sql = "update api_apollo_ticket set ticketStatus = ? where uuid=?";
            Db.update(sql, status, uuid);
            return;
        }
        String sql = "update api_apollo_ticket set ticketStatus = ?, ticketMsg=? where uuid=?";
